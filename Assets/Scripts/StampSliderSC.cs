...
public class StampSliderSC : MonoBehaviour {
	...
	void Start () {
		if (elements == null) {
			elements = Resources.LoadAll ("Stamps/Stamps", typeof(GameObject)).Cast<GameObject>().OrderBy(element => Int32.Parse (element.name)).ToArray();
			buttonImages = Resources.LoadAll ("Stamps/StampsButtons", typeof(Sprite)).Cast<Sprite>().ToArray();
